...
where
    S: Service<ServiceRequest, Response = ServiceResponse, Error = Error>, S::Future: 'static
{
    ...
    S: Service<ServiceRequest, Response = ServiceResponse, Error = Error>, S::Future: 'static
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        match is_logged(&req) {
            ...
            Err(err) => {
                Box::pin(async move {
                    Ok(ServiceResponse::new(req.into_parts().0, HttpResponse::Unauthorized().body(err)))
                })
            }
        }
    }
