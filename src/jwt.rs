...
pub fn generate(user : &User) -> String{
    ...
    jsonwebtoken::encode(&jsonwebtoken::Header::default(), &claims, &jsonwebtoken::EncodingKey::from_secret(&secret.as_bytes())).unwrap_or_default()
...
pub fn verify(token: String) -> Result<User,jsonwebtoken::errors::Error>{
    ...
    let token_data = jsonwebtoken::decode::<Claims>(&token, &jsonwebtoken::DecodingKey::from_secret(&secret.as_bytes()),&jsonwebtoken::Validation::new(jsonwebtoken::Algorithm::HS256))?;
